  ...
  try {
    ...
    if (
      !(
        ...
        typeof (metadata as Record<string, unknown>).ACLAddress === "string" &&
        (metadata as Record<string, unknown>).ACLAddress!.toString().startsWith("0x")
      )
    ) {
      return undefined;
    }
    if (
      !(
        ...
        typeof (metadata as Record<string, unknown>).InputVerifierAddress ===
          "string" &&
        (metadata as Record<string, unknown>).InputVerifierAddress!.toString().startsWith("0x")
      )
    ) {
      return undefined;
    }
    if (
      !(
        ...
        typeof (metadata as Record<string, unknown>).KMSVerifierAddress ===
          "string" &&
        (metadata as Record<string, unknown>).KMSVerifierAddress!.toString().startsWith("0x")
      )
    ) {
      return undefined;
    }
...
export const createFhevmInstance = async (parameters: {
  ...
  if (!sdkConfig) {
    throw new Error(
      "RelayerSDKLoader: Neither ZamaEthereumConfig nor SepoliaConfig is available. Please ensure the SDK is properly initialized."
    );
  }
